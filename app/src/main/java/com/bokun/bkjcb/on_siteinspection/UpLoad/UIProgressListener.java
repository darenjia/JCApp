...
public abstract class UIProgressListener implements ProgressListener{
    ...
    @Override
    public void onUpdate(long bytesWrite, long contentLength, boolean done) {
        //如果是第一次，发送消息
        if (!isFirst) {
            isFirst = true;
            Message start = Message.obtain();
            start.obj = new UploadTask(bytesWrite, contentLength, done);
        ...
        message.obj = new UploadTask(bytesWrite, contentLength, done);
        message.what = ProgressHandler.UPDATE;
        mHandler.sendMessage(message);

        if (done) {
            Message finish = Message.obtain();
            finish.obj = new UploadTask(bytesWrite, contentLength, done);
